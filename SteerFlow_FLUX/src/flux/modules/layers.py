...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor, pe: Tensor, info) -> Tensor:
        ...
        # Feature injection for editing
        if info["inject"] and info["id"] <= info["end_layer_index"] and info["id"] >= info["start_layer_index"]:
            v_name = str(info["t"]) + "_" + str(info["second_order"]) + "_" + str(info["id"]) + "_" + info["type"] + "_V"
            if info["inverse"]:
                if info["reuse_v"]:
                    info["feature"][v_name] = v.cpu()
            else:
                if info["reuse_v"] and v_name in info["feature"]:
                    v = info["feature"][v_name].cuda()
